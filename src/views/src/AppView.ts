...
export const AppView = () =>
{
    ...
    return _Structure(
        ...
        Div({className: 'w-full h-full flex flex-col items-center justify-center'},
            H1({
                className:   `text-7xl bg-black-gradient dark:bg-white-gradient
                              bg-clip-text text-transparent  mt-4 leading-[0.9] font-semibold tracking-tighter`,
                textContent: 'Wooper\'s Portfolio'
            }),
            H2({
                    className: 'text-3xl dark:text-white/60 text-black/60 mt-5 leading-[0.9] tracking-tighter',
                },
                'An', Span({
            ...
            Div({className: 'mt-5 p-10 gap-5 grid grid-cols-2 items-center justify-center overflow-auto'},
                _Loop(() => ls.cards,
                    (item) => Card({
                        header:      item?.header,
